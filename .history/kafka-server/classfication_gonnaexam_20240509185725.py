...
def test_feature_assembly(spark):
    ...
    df = spark.createDataFrame(data, ["EventType", "Timestamp", "Location", "Severity"])
    ...
    assembler = VectorAssembler(
        inputCols=["EventType_index", "Location_index", "Severity_index", "Timestamp_numeric"],
        outputCol="features"
    )
...
def test_anomaly_detection(spark):
    ...
    df = df.withColumn("severity_change", when(col("Severity") == "high", 2).otherwise(0))

    # Assert conditions
    results = df.collect()
    assert results[0]["severity_change"] == 0  # First entry has no previous severity
